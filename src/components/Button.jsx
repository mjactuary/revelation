...
export function Button({ className, arrow, children, isLoading, ...props }) {
  ...
  // Conditionally setting the classes based on the isLoading state
  const buttonClasses = isLoading
    ? 'inline-flex items-center justify-center bg-black-200 cursor-not-allowed'
    : 'text-white hover:bg-black-200';

  return typeof props.href === 'undefined' ? (
    <button className={`${baseClasses} ${buttonClasses}`} disabled={isLoading} {...props}>
      <ButtonInner arrow={arrow}>{isLoading ? 'Loading...' : children}</ButtonInner>
    </button>
  ) : (
